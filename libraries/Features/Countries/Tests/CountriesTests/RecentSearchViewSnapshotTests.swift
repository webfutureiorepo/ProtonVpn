...
//  along with Proton VPN.  If not, see <https://www.gnu.org/licenses/>.
    ...
    struct RecentSearchViewSnapshotTests {
        @Test("RecentSearchView")
        func recentSearchView() {
            let view = RecentSearchView(
                store: Store(
                    initialState: .init(recentSearches: ["United States", "Japan", "Sweden"])
                ) {
                    EmptyReducer()
                }
            )
    ...
    extension RecentSearchViewSnapshotTests: @preconcurrency AssertSnapshot {
