...
//  along with Proton VPN.  If not, see <https://www.gnu.org/licenses/>.
    ...
    struct NoResultsContentViewSnapshotTests {
        @Test("NoResultsContentView")
        func noResultsContentView() {
            let view = NoResultsContentView()
                .environment(\.colorScheme, .dark)
            assertSnapshot(of: view, as: .image(layout: .device(config: .iPhone13Mini)))
        }
    }

    extension NoResultsContentViewSnapshotTests: @preconcurrency AssertSnapshot {
