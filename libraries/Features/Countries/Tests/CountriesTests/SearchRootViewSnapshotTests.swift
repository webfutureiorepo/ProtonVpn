...
//  along with Proton VPN.  If not, see <https://www.gnu.org/licenses/>.
    ...
    struct SearchRootViewSnapshotTests {
        @Test("SearchRootView")
        func searchRootView() {
            let searchState = SearchFeature.State(searchResults: .placeholder)

            let view = NavigationStack {
                SearchRootView(
                    store: Store(initialState: .loaded(searchState)) {
                        EmptyReducer()
                    }
                )
            }
    ...
    extension SearchRootViewSnapshotTests: @preconcurrency AssertSnapshot {
