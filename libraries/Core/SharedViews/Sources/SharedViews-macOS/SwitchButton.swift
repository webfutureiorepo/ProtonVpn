...
public protocol SwitchButtonDelegate: AnyObject {
...
public extension SwitchButtonDelegate {
    func shouldToggle(_: NSButton, to _: ButtonState, completion: (Bool) -> Void) {
        completion(true)
    }
}
...
public class SwitchButton: NSView, CAAnimationDelegate {
    ...
    public var currentButtonState: ButtonState = .off {
        didSet {
    ...
    public var drawsUnderOverlay = true {
        didSet {
            initialSetup()
        }
    }
    ...
    public func registerDelegate(_ delegate: SwitchButtonDelegate) {
        self.delegate = delegate
        setState(currentButtonState, animated: false)
    }

    public func setState(_ state: ButtonState, animated: Bool = false) {
