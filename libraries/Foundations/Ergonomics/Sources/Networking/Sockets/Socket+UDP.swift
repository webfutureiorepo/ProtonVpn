...
//  along with Proton VPN.  If not, see <https://www.gnu.org/licenses/>.
    ...
    public extension Socket where Protocol == UDP {
        /// Returns the local endpoint the socket is bound to (IPv4).
        ...
        /// Returns the local endpoint the socket is bound to (IPv6).
        var localEndpointV6: sockaddr_in6 {
            get throws(SocketError) {
            ...
            if addressFamily == AF_INET6 {
                ...
                let bindResult = withUnsafePointer(to: &addr) {
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) {
                        bind(fd.fd, $0, socklen_t(MemoryLayout<sockaddr_in6>.size))
                    }
                }
    ...
    public extension Socket where Protocol == UDP, State == Opened {
        ...
        /// - Parameters:
        ///   - data: a ``Foundation.Data`` value.
        ///   - address: a ``sockaddr_in6`` value.
        func send(data: Data, to address: sockaddr_in6) throws(SocketError) {
            var address: sockaddr_in6 = address
            let result = data.withUnsafeBytes { bytes in
                withUnsafePointer(to: &address) {
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) {
                        sendto(
        ...
        ///   - buffer: a `UnsafeMutablePointer` value.
        ///   - bufferSize: the size of the buffer.
        ///   - senderAddr: an inout `sockaddr_in6` value that will contain the sender's address.
        ///   - noCopy: pass `true` if you want to receive a ``Foundation.Data`` built from a shared buffer. Defaults to `false`.
        func receive(
            buffer: inout UnsafeMutablePointer<UInt8>,
            bufferSize: Int,
            senderAddr: inout sockaddr_in6,
            noCopy: Bool = false
        ) throws(SocketError) -> Data {
